...
public final class Utils {
	...
	public static String getUniqueName(Nameable item, Iterable<? extends Nameable> items, boolean alwaysAppendNumber) {
		return getUniqueName(item.getName(), items, alwaysAppendNumber);
	}

	public static String getUniqueName(String name, Iterable<? extends Nameable> items) {
		return getUniqueName(name, items, false);
	}

	public static String getUniqueName(String name, Iterable<? extends Nameable> items, boolean alwaysAppendNumber) {
		String uniqueName = name;

		int counter = 1;
		if (alwaysAppendNumber) {
			uniqueName = name + "_" + counter;
			counter++;
		}
		while (containsName(uniqueName, items)) {
			uniqueName = name + "_" + counter;
			counter++;
		}
